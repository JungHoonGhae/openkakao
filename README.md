...
카카오톡에는 채널 API, 카카오 로그인 등 비즈니스용 API가 있지만, **개인 채팅에 프로그래밍으로 접근하는 방법은 제공되지 않는다.** Discord Bot API, Slack Webhook, Telegram Bot Framework처럼 내 채팅방의 메시지를 읽거나 보내는 공식 수단이 없다.
...
- 📖 **메시지** — 채팅 메시지 읽기, 전체 히스토리(`--all`), 검색, 내보내기(JSON/CSV/TXT)
- 📤 **메시지 전송** — 텍스트 + 사진/동영상/파일 전송 (LOCO SHIP+POST)
- 📡 **실시간 수신** — 메시지 실시간 모니터링, 자동 재연결, 읽음 처리(NOTIREAD)
- 📎 **미디어 다운로드** — 사진/동영상/음성/파일 첨부파일 다운로드, 실시간 자동 다운로드
- 👥 **친구** — 전체 목록, 즐겨찾기, 이름 검색, 숨김/해제
...
- 🩺 **진단** — 설치 상태, 토큰 유효성, 버전 드리프트, LOCO 연결 점검
...
# 확인 생략
openkakao-rs send <chat_id> "메시지 내용" -y
...
| 영역 | 조치 |
|------|------|
| **메모리** | LOCO 패킷/프레임 크기 상한(100MB) — 악성 서버의 OOM 공격 방지 |
| **암호화** | AES-128-GCM 랜덤 nonce, TLS 인증서 검증 (`webpki_roots`), RSA-OAEP SHA-1 |
| **파일** | 다운로드 파일명 sanitize (path traversal 차단), URL 도메인 allowlist |
| **자격 증명** | `0o600` 퍼미션으로 atomic 생성, 토큰 로그 8자 제한, 패스워드 비출력 |
| **입력 검증** | form body injection 방지 (URL 인코딩), 포트 범위 검증, DER/JPEG 파서 bounds check |

보안 이슈 발견 시 GitHub Issues로 보고해 주세요.

## 한계

- **macOS 전용** — 토큰 추출이 macOS의 NSURLCache(Cache.db)에 의존
- **비공식** — 카카오 서버 업데이트에 의해 언제든 동작이 중단될 수 있음
- **단일 세션** — LOCO 프로토콜은 하나의 디바이스 세션만 허용. `watch` 실행 중 다른 클라이언트(또는 카카오톡 앱)가 LOCO에 연결하면 기존 세션이 끊김
- **미디어 전송 제한** — `send-file`로 사진/동영상/파일 전송 가능하나, 썸네일 자동 생성 미지원
- **채팅방 생성 불가** — 새 채팅방 생성, 초대, 나가기 등 채팅방 관리 기능 미구현
- **REST 캐시 제한** — pilsner REST API는 최근 열었던 채팅방 메시지만 반환 (`loco-read`로 우회 가능)
- **오픈챗 제한** — 오픈채팅방 접근 시 `--force` 필수, 과도한 사용 시 계정 제재 위험
...
| LOCO 패킷 코덱 + 암호화 | 22B 헤더 + BSON, RSA-2048 OAEP + AES-128-GCM |
| 미디어 전송 | `send-file <chat_id> <file>` — 사진/동영상/파일 (LOCO SHIP+POST) |
| 미디어 다운로드 | `download <chat_id> <log_id>`, `watch --download-media` |
| 읽음 처리 (NOTIREAD) | `watch --read-receipt` |
| 자동 재연결 | `watch --max-reconnect N` — exponential backoff |
| 버전 드리프트 감지 | `doctor` — 설치 버전과 저장 자격 증명 버전 비교 |
...
| 우선순위 | 항목 | 비고 |
|----------|------|------|
| 높음 | 미디어 전송 썸네일 | 동영상/사진 전송 시 자동 썸네일 생성 |
| 중간 | TUI 모드 | `ratatui` 기반 터미널 UI |
| 중간 | Webhook/Hook 시스템 | 메시지 수신 시 쉘 스크립트/webhook 트리거 |
| 중간 | Matrix 브릿지 | matrix-appservice-kakaotalk 연동 |
| 낮음 | macOS 알림 연동 | `watch` 모드에서 네이티브 알림 |
| 낮음 | Chat DB 복호화 | SQLCipher key derivation |
