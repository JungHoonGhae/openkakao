...
OpenKakao는 "메시지 전송"은 없지만, **조회·분석·자동 리포팅** 계열에는 충분히 쓸 수 있다.

### 프로젝트 예시

1. **개인 대시보드**  
   오늘 안 읽은 채팅 수, 최근 대화방 TOP N, 자주 대화하는 사람을 매일 집계
2. **아침 브리핑 봇**  
   전날 밤~아침까지의 중요한 메시지(키워드 기반)를 요약해서 노션/슬랙으로 전달
3. **채팅 검색 CLI 확장**  
   특정 유저·키워드·기간으로 메시지 필터링하는 서브커맨드 추가
4. **오픈채팅 모니터링 툴**  
   특정 방의 신규 메시지를 주기적으로 읽어 이슈/질문 패턴 분석
5. **링크 아카이브 파이프라인**  
   대화 중 공유된 URL을 수집하고 제목/설명을 붙여 문서화
6. **관계 그래프 분석**  
   멤버/대화 빈도를 그래프로 만들어 커뮤니케이션 패턴 시각화
7. **세금·업무 증빙 정리 도우미**  
   특정 키워드(견적, 입금, 계약 등) 포함 메시지를 주기적으로 추출해 CSV 저장
8. **읽기 전용 AI 어시스턴트**  
   최근 메시지 맥락을 가져와 "무슨 얘기 중이었는지" 요약해주는 로컬 도구

### 잘 맞는 자동화 패턴

- `cron + openkakao-rs`: 정기 수집
- `openkakao-rs + jq`: JSON 가공
- `openkakao-rs + sqlite/postgres`: 메시지 인덱싱
- `openkakao-rs + LLM`: 요약/분류/태깅
...
Rust 릴리스/탭 자동화 절차는 아래 문서를 참고:

- [docs/RELEASING_RUST.md](docs/RELEASING_RUST.md)

핵심 포인트:
- 릴리스 태그: `openkakao-rs-vX.Y.Z`
- GitHub Actions secret: `HOMEBREW_TAP_TOKEN` 필요 (tap 저장소 push 권한 토큰)
