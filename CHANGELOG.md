...
### Added
- **`local-send-photo <chat_name> <file>`** — send one image through the official signed KakaoTalk app via Accessibility, with no server/LOCO login. The image is validated by content (JPEG/PNG/GIF/WebP/HEIF magic bytes, 1 byte–50 MiB, regular file) and the path canonicalized and required to be valid UTF-8 before anything is driven. The native file picker is driven primarily through semantic AX controls (the unique Send Files ⌘O button, Go to Folder path entry, selected-filename read-back), with focus-verified coordinate fallbacks; pressing Open only advances the picker to KakaoTalk's own attachment preview, and after macOS device-owner authentication the recipient chat window and previewed filename are revalidated immediately before the final Send press, with delivery confirmed only by one additional outgoing `[사진]` bubble. An explicit unattended authorization (`--unattended` together with `--allow-non-interactive-send`, or `[mode] unattended = true` + `[send] allow_non_interactive = true` in config) skips the device-owner prompt for scheduled photo sends under the same `safety.min_unattended_send_interval_secs` throttle as other unattended sends. It shares `local-send`'s `allow_ax_send` + exact `allowed_send_chats` gates (skipped for `--dry-run`), refuses to disturb an existing composer draft, and reports any ambiguous post-commit outcome as `uncertain` so callers never retry blindly.
