...
| session recovery for LOCO failures | `relogin` first, `renew` second | login.json tends to restore a fresher access token than passive renewal |
...
For service stability, the CLI now treats recovery in this order:
...
That is the practical boundary today. REST and LOCO both reuse the same recovery ladder, but LOCO remains the stricter path because it needs a live session, not just a valid bearer token.
