...
| `local-send-photo <chat_name> <file>` | Send one validated image (JPEG/PNG/GIF/WebP/HEIF, max 50 MiB) through the same AX path and KakaoTalk's native file picker (same config gates, supports `--dry-run`) |
