...
Allowlist the chat you intend to send to (required before any real send — `local-send`/`local-send-photo` have no chat-id to cross-check against, so this is the only guard against sending to the wrong chat):
...
# Preview a photo send, then send it through the official app
...
See [local-send / local-send-photo / ax-read](/docs/cli/local-send) for the full reference.
