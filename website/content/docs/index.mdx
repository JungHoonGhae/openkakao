...
<Callout title="Want to evaluate fit first?">
  Start with [Automation Overview](/docs/automation/overview) if you are deciding whether OpenKakao is useful. Move to [Trust Model](/docs/security/trust-model) once you need the risk boundary.
</Callout>
...
<Callout title="Keep the model small">
  OpenKakao is not a hosted messaging platform. It is a local bridge. Treat it as a way to inspect, structure, and narrow automation around a real KakaoTalk session.
</Callout>
